...
describe('resolveDownloadOptions', () => {
  it('returns a combined audio+video stream for mp4 highest', () => {
  ...
  it('returns the lowest combined stream for mp4 lowest', () => {
    expect(resolveDownloadOptions('mp4', 'lowest')).toEqual({
      filter: 'audioandvideo',
      quality: 'lowest',
      container: 'mp4'
    });
...
describe('buildOutputPath', () => {
  ...
  it('builds a simple path when nothing collides', () => {
    ...
    expect(result).toBe('/out/My Video.mp4');
  });

  it('appends a counter when the target already exists', () => {
    const taken = new Set(['/out/My Video.mp4', '/out/My Video (1).mp4']);
    const result = buildOutputPath(join, '/out', 'My Video', 'mp4', c =>
      taken.has(c)
    );
    expect(result).toBe('/out/My Video (2).mp4');
  });
});
